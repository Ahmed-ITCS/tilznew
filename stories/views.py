...
class StoryListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Story.objects.filter(visibility='public').select_related('author')
...
# Change from ReadOnlyModelViewSet to ModelViewSet to allow POST requests
class StoryViewSet(viewsets.ModelViewSet):
    ...
    # Remove this method as it's now handled in the serializer
...
class EpisodeViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['post'])
    def branch(self, request, story_pk=None, pk=None):
        """Create a branched episode from an existing episode"""
        try:
            original_episode = self.get_object()
            story = Story.objects.get(pk=story_pk)
            
            if story.status != 'active':
                return Response(
                    {'detail': 'Cannot branch episodes from inactive or quarantined stories'},
                    status=status.HTTP_403_FORBIDDEN
                )
            
            serializer = EpisodeCreateSerializer(
                data=request.data,
                context={
                    'story': story,
            ...
            return Response(
                {'detail': 'Original episode not found'},
                status=status.HTTP_404_NOT_FOUND
            )
